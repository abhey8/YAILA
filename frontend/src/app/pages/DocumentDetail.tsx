...
export default function DocumentDetail() {
  ...
  useEffect(() => {
    if (!id || !isDocumentReady || isTopicFlow || document?.summaryStatus !== "generating") {
      return;
    }
    ...
    const intervalId = window.setInterval(async () => {
      try {
        ...
        if (latest?.summaryStatus === "ready" || latest?.summaryStatus === "failed") {
          window.clearInterval(intervalId);
        }
      } catch (_) {
      }
    ...
    return () => window.clearInterval(intervalId);
  }, [id, isDocumentReady, isTopicFlow, document?.summaryStatus]);

  useEffect(() => {
    if (!id || !isDocumentReady || activeTab !== "summary" || isTopicFlow) {
      return;
    }
    if (document?.summaryStatus === "ready" && document?.summary) {
      return;
    }
    if (document?.summaryStatus === "generating") {
      return;
    }

    handleGenerateSummary(false, true);
  }, [id, isDocumentReady, activeTab, isTopicFlow, document?.summaryStatus]);
  ...
  const handleGenerateSummary = async (regenerate = false, silent = false) => {
    ...
    try {
      ...
      setDocument((current: any) => current ? {
        ...current,
        summary: response.summary || current.summary || "",
        summaryStatus: response.status === "ready" ? "ready" : "generating",
        summaryError: response.error || null,
        summaryUpdatedAt: response.updatedAt || current.summaryUpdatedAt,
      } : current);
      ...
      if (!silent) {
        if (response.status === "ready") {
          ...
          toast.success(regenerate ? "Summary regeneration started in the background" : "Summary generation started in the background");
        }
      }
    } catch (error) {
      if (!silent) {
                ...
                {summary ? (
                  <div className="p-6 rounded-lg border border-[var(--accent-primary)]/20 bg-[var(--accent-soft)]">
                    {document?.summaryStatus === "generating" ? (
                      <div className="mb-4 rounded-lg border border-[var(--border)] bg-[var(--surface-1)] px-4 py-3 text-sm text-[var(--foreground-soft)]">
                        Summary is still being improved in the background. You can keep studying and it will stay saved here.
                      </div>
                    ) : null}
                  ...
                  <div className="text-center py-12">
                    ...
                    <p className="text-[var(--muted-foreground)] mb-4">
                      {document?.summaryStatus === "generating"
                        ? "Your summary is being prepared in the background. You can switch tabs and come back."
